"""
飞书智能对话 Agent
使用豆包 function_calling，理解自然语言指令并调用飞书各项功能
"""
import json
import logging
import os
...
# ── 工具定义（OpenAI function_calling 格式）────────────────────────────────────
TOOLS = [
    {
        "type": "function",
        "function": {
...
class FeishuChatAgent:
    ...
    def chat(
        ...
        """
        # 检测历史损坏：末尾是 tool 消息说明上次调用中途失败，重置历史
        if self._history and self._history[-1]["role"] == "tool":
        ...
        try:
            ...
            # 保持历史在合理长度（最近 60 条），从完整 user 消息开始
