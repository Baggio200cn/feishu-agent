...
# ── agent_cli.py 辅助脚本（对话模式入口）────────────────────────────────────

def _ensure_agent_cli():
    """生成 agent_cli.py（供 ChatWindow 子进程调用）"""
    cli_path = os.path.join(BASE_DIR, "agent_cli.py")
    if os.path.exists(cli_path):
        return
    code = '''"""
agent_cli.py — ChatWindow 子进程入口
